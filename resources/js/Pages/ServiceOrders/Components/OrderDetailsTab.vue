...
<template>
    <!-- Agregamos w-full y overflow-x-auto al contenedor principal -->
    <div class="p-2 md:p-4 w-full overflow-x-auto">
        <!-- Forzamos un ancho mínimo para móviles, permitiendo el scroll horizontal -->
        <div class="min-w-[800px] xl:min-w-full">
            <n-descriptions label-placement="top" bordered column="3">
                ...
                <n-descriptions-item label="Fecha de Finalización">
                    <div class="flex items-center gap-2"
                        :class="order.completion_date ? 'text-green-600 font-medium' : 'text-gray-400'">
                        <n-icon>
                ...
                <n-descriptions-item label="Representante de Ventas">
                    <div v-if="order.sales_rep" class="flex items-center gap-3">
                        <n-avatar size="small" round :src="order.sales_rep.profile_photo_url"
                            :fallback-src="'https://ui-avatars.com/api/?name=' + order.sales_rep.name" />
                ...
                <n-descriptions-item label="Número de Servicio / N° Medidor">
                    <div v-if="order.service_number"
                        class="flex items-center gap-2 font-mono text-indigo-700 bg-indigo-50 px-2 py-1 rounded w-fit">
                        <n-icon>
                            <FlashOutline />
                        </n-icon> {{ order.service_number }} / {{ order.meter_number }}
                    </div>
            ...
            <!-- SECCIÓN DE ESPECIFICACIONES ELÉCTRICAS -->
            <h4 class="font-bold text-gray-700 mt-8 mb-4 border-b pb-2 flex items-center gap-2">
                <n-icon class="text-amber-500">
                    <FlashOutline />
                </n-icon> Especificaciones Eléctricas
            </h4>

            <n-descriptions label-placement="top" bordered column="4">
                <n-descriptions-item label="Voltaje">
                    <span v-if="order.voltage" class="font-medium text-gray-800">{{ order.voltage }}</span>
                    <span v-else class="text-gray-400 italic">-</span>
                </n-descriptions-item>

                <n-descriptions-item label="Número de Hilos">
                    <span v-if="order.number_of_wires" class="font-medium text-gray-800">{{ order.number_of_wires
                        }}</span>
                    <span v-else class="text-gray-400 italic">-</span>
                </n-descriptions-item>
                ...
                <n-descriptions-item label="Capacidad Total (Generación)" :span="4">
                    <div class="bg-indigo-50 border border-indigo-100 rounded-lg p-3 inline-flex items-center gap-3">
                        <div class="bg-indigo-600 text-white rounded-full p-1.5 flex items-center justify-center">
                            <n-icon>
                                <FlashOutline />
                            </n-icon>
                        </div>
                        <div>
                            <div class="text-xs text-indigo-800 uppercase font-bold tracking-wider">Potencia Total</div>
                            <div class="text-2xl font-black text-indigo-700">
                                {{ order.total_capacity ? order.total_capacity : '0.00' }} <span
                                    class="text-sm font-medium">KW</span>
                            </div>
                        </div>
                    </div>
                </n-descriptions-item>

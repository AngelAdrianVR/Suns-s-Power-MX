...
const assignedProducts = ref([]); // Productos YA asignados (Catálogo)
const isLoadingProducts = ref(false);

// Estado del Modal de Asignación Rápida
const showAssignModal = ref(false);
const candidateProducts = ref([]); // Productos globales buscados
...
const form = useForm({
    supplier_id: null,
    expected_date: null,
    currency: 'MXN', // Moneda de la orden
    notes: '',
    items: [], // Array de { product_id, quantity, unit_cost, ... }
});
...
// --- 1. CARGA DE PRODUCTOS ASIGNADOS ---
watch(() => form.supplier_id, async (newSupplierId) => {
    form.items = []; // Limpiar items
    ...
    try {
        // Usamos la NUEVA ruta que devuelve los asignados con sus precios pactados
        // IMPORTANTE: Asegúrate de registrar esta ruta en web.php apuntando a PurchaseOrderController@getSupplierProducts
        const response = await axios.get(route('suppliers.products.assigned', newSupplierId));
        
        assignedProducts.value = response.data.map(p => ({
            label: p.name,
            value: p.id,
            sku: p.sku,
            // Datos pivote cruciales
...
const addItem = (preselectedProductId = null) => {
    // Si pasamos un ID, buscamos sus datos para precargar
    let initialCost = 0;
    ...
    if (preselectedProductId) {
        ...
        if (product) {
            ...
            // Opcional: Advertir si la moneda no coincide
            if (product.pivot_currency !== form.currency) {
                notification.warning({ 
                    title: 'Diferencia de Moneda', 
                    content: `Este producto está registrado en ${product.pivot_currency}, pero la orden es en ${form.currency}. Verifica el precio.`,
                    duration: 5000 
                });
            }
        }
...
const handleProductChange = (val, index) => {
    ...
    if (product) {
        // Llenamos precio automáticamente desde el registro del proveedor
        form.items[index].unit_cost = product.pivot_price;
        
        // Validación visual de moneda
        if (product.pivot_currency && product.pivot_currency !== form.currency) {
             notification.info({ 
                content: `Precio sugerido en ${product.pivot_currency}. Ajustado a la orden (${form.currency}).`,
                duration: 2000
            });
        }
    }
...
// --- 3. ASIGNACIÓN RÁPIDA (NUEVOS PRODUCTOS) ---
const openAssignModal = () => {
    if (!form.supplier_id) {
        notification.warning({ content: 'Primero selecciona un proveedor.' });
        return;
    }
    ...
    quickAssignForm.value = {
        ...
        product_obj: null,
        purchase_price: 0,
        currency: form.currency, // Sugerir la misma moneda de la orden
        delivery_days: 1,
        supplier_sku: ''
    };
    ...
    // Cargar sugerencias iniciales (opcional)
...
const searchGlobalProducts = (query) => {
    ...
    searchTimeout.value = setTimeout(async () => {
        ...
        try {
            // Usamos la ruta existente que busca productos NO asignados (o globales)
            // Se asume que 'suppliers.products.fetch' acepta parámetros de búsqueda o devuelve lista
            const response = await axios.get(route('suppliers.products.fetch', form.supplier_id), {
                params: { query: query } // Asumiendo que el backend filtra si se envía query
            });
            
            // Filtramos localmente si el backend devuelve todo, o usamos la respuesta directa
...
const selectCandidate = (val, option) => {
    ...
    quickAssignForm.value.purchase_price = parseFloat(option.raw.price || 0); // Precio base del producto
...
const submitQuickAssign = async () => {
    ...
    try {
        ...
        // 3. Agregarlo inmediatamente como una fila en la orden
        addItem(newProduct.value);

        showAssignModal.value = false;

    } catch (error) {
        notification.error({ title: 'Error', content: 'No se pudo asignar el producto.' });
    }
};
    ...
    formRef.value?.validate((errors) => {
        if (!errors) {
            if(form.items.some(i => !i.product_id || i.quantity <= 0)) {
                notification.warning({ title: 'Incompleto', content: 'Revisa las partidas.' });
                return;
            }
                        ...
                        <div class="lg:col-span-8 space-y-6">
                            <n-card :bordered="false" class="shadow-sm rounded-2xl min-h-[500px]">
                                ...
                                <div v-if="form.items.length === 0" class="py-12 flex flex-col items-center justify-center text-gray-400 text-center border-2 border-dashed border-gray-100 rounded-xl bg-gray-50/50">
                                    ...
                                    <p v-else>Agrega productos a la lista o asigna nuevos del catálogo general.</p>
                                </div>

                                <!-- Spinner Carga -->
                                <div v-if="isLoadingProducts" class="py-8 text-center">
                                    <n-spin size="medium" /> 
                                    <p class="text-xs text-gray-500 mt-2">Cargando precios pactados...</p>
                                </div>
                    ...
                    >
                        ...
                        <div class="space-y-4">
                            <p class="text-sm text-gray-500">
                                Busca un producto del inventario global y define el precio acordado para agregarlo a esta orden.
                            </p>
